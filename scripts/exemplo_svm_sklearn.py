...
##### SVM (support-vector machine) from Sklearn #####
from sklearn.svm import LinearSVC
# passar parâmetros
svm = LinearSVC(max_iter=500, random_state=1, C=1000)
# treinar com os dados
svm.fit(X, y)

# plotar dados, superfície de decisão e margem
plot_decision_regions(X=X, y=y, classifier=svm, test_idx=None)
plt.xlabel('x1')
plt.ylabel('x2')
plt.title("SVM")
plt.savefig("figures/iris_sklearn_svm.png")